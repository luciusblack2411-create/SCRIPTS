...
def test_genie_accepts_precollected_output_and_returns_expected_structure() -> None:
    content = FIXTURE.read_text(encoding="utf-8")
    parsed = IOSShowInterfacesStatusParser().extract_genie_structure(content)

    assert parsed == EXPECTED_GENIE
    assert len(parsed["interfaces"]) == 7
    assert len(set(parsed["interfaces"])) == 7
...
def test_adapter_preserves_raw_and_builds_framework_model_and_evidence() -> None:
    ...
    original_bytes = raw.content.encode(raw.encoding)
    ...
    assert evidence_by_field["interfaces[0].status"].line_start == 2
    assert evidence_by_field["interfaces[3].status"].line_start == 5
