...
- **A0 — Baseline freeze: documented in this change.** The two functional identities, reused stable IDs, current flow, authority limits, evidence hierarchy, fallback, and known gaps are recorded with regression coverage. No runtime behavior changes.
- **A1 — Intake ergonomics: gap, not implemented.** Human-friendly creation and validation of feature-intake artifacts is not yet a unified guided local workflow; explicit human contract approval must remain separate.
- **A2 — Local run command and recovery UX: gap, not implemented.** Existing typed controller/runtime surfaces and journals are present, but a cohesive operator experience for start, inspect, resume, and base-refresh recovery remains future work.
- **A3 — Evidence presentation: gap, not implemented.** CI, review, journal, and live-ref evidence exists across typed outputs; consolidated human-readable status and provenance views remain future work.
- **A4 — Draft PR amendment loop: gap, not implemented.** A bounded amendment control plane exists, but end-to-end orchestration of review feedback, renewed scope confirmation, amendment, and re-review remains future work.
- **A5 — Operational hardening: gap, not implemented.** Credential lifecycle guidance, recovery playbooks, observability, packaging validation, and broader failure drills need further definition without weakening least privilege or fail-closed behavior.
- **A6 — Adoption and governance: gap, not implemented.** Rollout criteria, operator training, audit review, compatibility policy, and measurable acceptance gates remain future work. This milestone cannot introduce autonomous authority.
