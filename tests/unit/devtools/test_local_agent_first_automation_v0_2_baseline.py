...
def test_baseline_records_each_roadmap_marker_without_implementing_later_steps() -> None:
    text = _baseline()
    for marker in range(7):
        assert re.search(rf"^- \*\*A{marker} — ", text, re.MULTILINE)
    for marker in range(1, 7):
        assert re.search(
            rf"^- \*\*A{marker} — .*: gap, not implemented\.\*\*",
            text,
            re.MULTILINE,
        )
